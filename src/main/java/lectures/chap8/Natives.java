...
public class Natives {
    public ExecEnvStorage environment(ExecEnvStorage env) {
        appendNatives(env);
        return env;
    }
    protected void appendNatives(ExecEnvStorage env) {
    ...
    protected void append(ExecEnvStorage env, String name, Class<?> clazz,
                          String methodName, Class<?> ... params) {
        Method m;
        try {
            m = clazz.getMethod(methodName, params);
        } catch (Exception e) {
            throw new CognacException("cannot find a native function: "
                                     + methodName);
        }
